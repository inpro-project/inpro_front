...
<button style="border:solid; border-radius:10px; border-width:0px; background-color:gray; color:white; width:50%; height:30px" @click="addPortfolio">수상 경력 등록</button>
  ...
  methods: {
    getportfolioinfodata () {
      ...
      // get 속 링크의 useridx값은 카카오 로그인 후 부여받는 useridx 고유값 => http://prod.inpro-server.shop:9000/app/portfolios/:userIdx/:portfoliCategoryIdx
        .get(process.env.VUE_APP_API_BASE_URL + '/app/portfolios/' + process.env.VUE_APP_USER_ID + '/2', { headers: { 'Content-Type': 'application/json', Authorization: process.env.VUE_APP_ACCESS_TOKEN } })
        .then(res => {
    ...
    async removePortfolio (portfolio) {
      ...
      this.$router.go({ name: 'fixwinlist' })
    },
    ...
    postPortfolio (portfolio) {
      ...
      axios
        .post(process.env.VUE_APP_API_BASE_URL + '/app/portfolios/2', JSON.stringify(data), { headers: { 'Content-Type': 'application/json', Authorization: process.env.VUE_APP_ACCESS_TOKEN } })
        .then(res => {
          console.log(res)
        })
        .catch(err => {
