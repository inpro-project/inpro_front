<template>
    <!--1대팀보여주기-->
    <OtherTeaminfoViewVue :key="componentKey"/>
...
export default {
  ...
  methods: {
    ...
    goBack () { // 이전으로 되돌리기
      if (this.$store.state.iteamlike[this.$store.state.isteamlike.length - 1] !== '0') { // 직전 islike에 값이 있음 => 직전에 좋아요를 눌렀다는 뜻
        this.deleteLike()
        this.$store.state.isteamlike.pop() // 직전 요소 모두 pop
        this.$store.state.isteampass.pop()
      } else if (this.$store.state.isteampass[this.$store.state.isteampass.length - 1] !== '0') {
        this.deletepass()
        this.$store.state.isteamlike.pop()
        this.$store.state.isteampass.pop()
      }
      this.$router.go(-1)
      this.subCounter()
    ...
    async sendLike () { // 유저 좋아요
      ...
      this.$store.state.isteamlike.push(likingIdx)
      this.$store.state.isteampass.push('0')
      this.addCounter()
      this.nextpage()
    },
    ...
    async passLike () { // 유저 좋아요 넘기기
      ...
      this.$store.state.isteampass.push(passingIdx)
      this.$store.state.isteamlike.push('0')
      this.addCounter()
      this.nextpage()
    },
    ...
    addCounter () { // 조회수 카운트 +1
      if (this.$store.state.teamcounter < this.$store.state.teamIdx.length) {
        this.$store.state.teamcounter++
      } else { this.$store.state.teamcounter = 0 }
    },
    subCounter () { // 조회수 카운트 -1
      if (this.$store.state.teamcounter > 0) {
        this.$store.state.teamcounter--
      } else { this.$store.state.teamcounter = 0 }
    },
    nextpage () {
    ...
    loadidx () {
      this.nowIdx = this.$store.state.teamIdx[this.$store.state.teamcounter]
