...
export default {
  ...
  methods: {
    getuserinfodata () {
      axios
        .get(process.env.VUE_APP_API_BASE_URL + '/app/matched-users', { headers: { 'Content-Type': 'application/json', Authorization: VueCookies.get('Authorization') } })
        .then(res => {
    ...
    gotoprofile (idx) {
      ...
      this.$router.push({ name: 'matcheduserinfo', params: { userIdx: this.$store.state.otheruserIdx } })
    },
    checkLogin () {
